...
namespace Fuse.Controls
{
	partial class MobileTextEdit: TextEdit, ITextEditHost, INotifyFocus
	{
		...
		{
			if defined(Android)
				return TextEditRenderer.NewRenderer(_isMultiline);
			else if defined(iOS)
	...
	{
		public static IViewHandleRenderer NewRenderer(bool isMultiline)
		{
			return new Renderer(isMultiline);
		}
		...
		class Renderer : IViewHandleRenderer
		{
			...
			public Renderer(bool isMultiline)
			{
			...
			bool isMultiline)
		{
			...
			CopyState(viewHandle.NativeHandle, _renderView.NativeHandle, firstFrame, isMultiline, pixelSize.X, pixelSize.Y);
			renderer.Draw(_renderView, localToClipTransform, position, size, density);
		}

		[Foreign(Language.Java)]
		static void CopyState(Java.Object sourceHandle, Java.Object targetHandle, bool firstFrame, bool isMultiline, int width, int height)
		@{
